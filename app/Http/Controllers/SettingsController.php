...
class SettingsController extends Controller
{
    ...
    private $promises = [];
    private $paramData = [];
    function createArray($value, $column)
    {
        $arr = [];
        foreach (array_slice($value, 1) as $k => $val) {
            $arr[$column[$k]] = $val;
        }
        return $arr;
    }
    public function index()
    {
        ...
        if (count($files) > 0) {
            ...
            foreach ($files as $fi => $file) {
                ...
                // $hourly = ['tenant_name','sales_date','hourly_sales','gross_sales','vat_amount','adjustment','service_charge','no_transaction','sales_department','no_refund','refund_amount','no_void','void_amount','gross_vatexempt'];
                // $daily = ['tenant_name','sales_date','gross_sales','vat_amount','adjustment','service_charge','no_transaction','sales_department','no_refund','refund_amount','no_void','void_amount','gross_vatexempt'];
                // $grandTotal = ['tenant_name','sales_date','old_grandTotal', 'new_grandTotal'];

                // $file = fopen($uri."/".$filename,"r");
                // while(! feof($file))
                // {
                ...
                // fclose($file);
                // $jsonVal['filename'] = substr($filename,0,-4);
                // foreach($array as $key => $value){
                //     if($value[0] == '01'){
                //         $jsonVal['hourly'][]=$this->createArray($value, $hourly);
                //     }else if($value[0] == '99'){
                //         $jsonVal['daily'][]=$this->createArray($value, $daily);
                //     }else{
                //         $jsonVal['grandTotal'][]=$this->createArray($value, $grandTotal);
                //     }
                // }

                // dd(json_encode($jsonVal));
                // die;
