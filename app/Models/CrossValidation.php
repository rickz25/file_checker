    ...
    public function validateSales($params)
    {
        ...
        try {
            $cross_query = DailyValidation::select(
                ...
                DB::raw("(SELECT printf('%.2f', SUM(CAST(GROSS_SLS AS DECIMAL(10,2)))) from transaction_validations WHERE CCCODE='$CCCODE' AND TRN_DATE='$TRN_DATE' AND TER_NO ='$TER_NO') as gross_trans"),
                ...
                DB::raw("printf('%.2f', SUM(CAST(GROSS_SLS AS DECIMAL(10,2))) - (SELECT SUM(CAST(GROSS_SLS AS DECIMAL(10,2))) from transaction_validations WHERE CCCODE='$CCCODE' AND TRN_DATE='$TRN_DATE' AND TER_NO ='$TER_NO')) as gross"),
                ...
                DB::raw("printf('%.2f', SUM(CAST(abs((CASH_SLS + OTHER_SLS + CHECK_SLS + GC_SLS + MASTERCARD_SLS + VISA_SLS + AMEX_SLS + DINERS_SLS + JCB_SLS + GCASH_SLS + PAYMAYA_SLS + ALIPAY_SLS + WECHAT_SLS + GRAB_SLS + FOODPANDA_SLS + MASTERDEBIT_SLS + VISADEBIT_SLS + PAYPAL_SLS + ONLINE_SLS + OPEN_SALES + OPEN_SALES_2 + OPEN_SALES_3 + OPEN_SALES_4 + OPEN_SALES_5 + OPEN_SALES_6 + OPEN_SALES_7 + OPEN_SALES_8 + OPEN_SALES_9 + OPEN_SALES_10 + OPEN_SALES_11) - GC_EXCESS) AS DECIMAL(10,2))) - (SELECT SUM(CAST(abs((CASH_SLS + OTHERSL_SLS + CHECK_SLS + GC_SLS + MASTERCARD_SLS + VISA_SLS + AMEX_SLS + DINERS_SLS + JCB_SLS + GCASH_SLS + PAYMAYA_SLS + ALIPAY_SLS + WECHAT_SLS + GRAB_SLS + FOODPANDA_SLS + MASTERDEBIT_SLS + VISADEBIT_SLS + PAYPAL_SLS + ONLINE_SLS + OPEN_SALES + OPEN_SALES_2 + OPEN_SALES_3 + OPEN_SALES_4 + OPEN_SALES_5 + OPEN_SALES_6 + OPEN_SALES_7 + OPEN_SALES_8 + OPEN_SALES_9 + OPEN_SALES_10 + OPEN_SALES_11)-GC_EXCESS) AS DECIMAL(10,2))) from transaction_validations WHERE CCCODE='$CCCODE' AND TRN_DATE='$TRN_DATE' AND TER_NO ='$TER_NO')) as payment")
            )
